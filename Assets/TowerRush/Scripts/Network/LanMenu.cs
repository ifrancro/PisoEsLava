    ...
    public sealed class LanMenu : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            if (!connected)
            {
                ...
                useInternet = GUILayout.Toolbar(useInternet ? 0 : 1, ConnectionModes, GUILayout.Height(30f)) == 0;
                GUILayout.Label(useInternet ? "2 a 4 jugadores. Compartí código, desde cualquier red."
                    : "2 a 4 jugadores en la misma red.", textStyle);
                GUILayout.Space(8f);
                ...
                GUILayout.Space(14f);
                GUILayout.Label(useInternet ? "Código de partida" : "IP del host", textStyle);
                if (useInternet) joinCode = GUILayout.TextField(joinCode, 12, GUILayout.Height(30f));
                else hostAddress = GUILayout.TextField(hostAddress, 45, GUILayout.Height(30f));
                if (GUILayout.Button("Unirse a la partida", GUILayout.Height(40f))) JoinMatch();
                GUI.enabled = true;
                if (!useInternet) GUILayout.Label("Puerto UDP: " + port, textStyle);
